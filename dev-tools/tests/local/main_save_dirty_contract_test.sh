...
require 'let MAIN_PAGE_SAVE_DIRTY = false;' "$UI_FILE" 'main-page dirty state must be cached outside the render path'
require "const mainPageDirty = state === 'ready' && MAIN_PAGE_SAVE_DIRTY;" "$UI_FILE" 'authority rendering must use cached dirty state'
require 'MAIN_PAGE_SAVE_DIRTY = settingsAuthorityIsReady() && mainPageSaveIsDirty();' "$UI_FILE" 'dirty state is not refreshed after an authoritative form update'
reject "const mainPageDirty = state === 'ready' && mainPageSaveIsDirty();" "$UI_FILE" 'authority rendering re-enters form serialization'
