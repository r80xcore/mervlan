...
legend_items=$(sed -n '/<!-- Legend -->/,/<!-- Settings \/ experimental row -->/p' "$UI_FILE" | grep -Fc 'class="legend__item"')
...
grep -Fq 'title="Yellow dot in a VLAN input: this VLAN ID is used more than once. Reuse is allowed when intentional."' "$UI_FILE" || fail 'Duplicate legend title mismatch'
