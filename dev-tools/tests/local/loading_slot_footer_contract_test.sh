...
require_ui 'function resumePollingAfterBrowserResume()' 'loading coordinator has no browser-resume polling path'
require_ui "window.addEventListener('pageshow', resumePollingAfterBrowserResume);" 'loading coordinator does not repoll after pageshow'
require_ui "document.addEventListener('visibilitychange'" 'loading coordinator does not repoll after visibility return'
require_ui "const MERV_LOADING_RESUME_STORAGE_KEY = 'mervlan.loading.resume.v1';" 'reload-resume storage key missing'
require_ui 'function persistProgressAction(task)' 'running backend action is not persisted for reload recovery'
require_ui 'function resumePersistedProgressAction()' 'reload cannot reattach to a persisted backend action'
require_ui "window.addEventListener('pageshow', resumePersistedProgressAction);" 'reload-resume hook is not registered on pageshow'
